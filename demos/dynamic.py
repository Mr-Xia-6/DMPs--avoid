...
for n in range(num_obst_1):
    obst = point_obstacle.obstacle(x_obst = np.array([x_c_1 + a_1*np.cos(t_1[n]), y_c_1 + b_1*np.sin(t_1[n])]), dx_obst = [-0.004,0.0035])
...
while (not flag):
    ...
    # for n in range(num_obst_1):
    #     obst = point_obstacle.obstacle(x_obst=np.array([x_c_1 + a_1 * np.cos(t_1[n]), y_c_1 + b_1 * np.sin(t_1[n])]),
    #                                    dx_obst=[-0.004, 0.0035])
    #     obst_list_1.append(obst)

    for n in range(num_obst_1):
        obst = point_obstacle.obstacle(x_obst=np.array([x_c_1 + a_1 * np.cos(t_1[n])-0.004*numv, y_c_1 + b_1 * np.sin(t_1[n])+0.0035*numv]),
                                       dx_obst=[-0.004, 0.0035])
        f_n = obst.gen_external_force(dmp.x, dmp.dx, dmp.goal)
        F_1 += f_n
    F = F_1+F_2
    numv=numv+1
    ...
    for n in range(num_obst_1):
        ...
        f2=-np.dot(R1,dmp.dx)*0.665*normd*jiaodu*np.exp(0.45*weizhi)*np.exp(0.85*jiaodu)
        F_2 += f2*jiaodu
    # print(F_2)
    # print(f2)
    number = number + 1
    x_plot_1 = x_plot_1-0.004
    y_plot_1 = y_plot_1+0.0035

    plt.plot(x_plot_1, y_plot_1, ':b', lw=2, label='obstacle')
    plt.plot(x_plot_1, y_plot_1, ':w', lw=2, label='obstacle')
    plt.xlim(-1, 0.38)
    plt.ylim(-0.1, 0.8)
